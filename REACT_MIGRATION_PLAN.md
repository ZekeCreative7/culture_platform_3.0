...
### 2026-07-04 - Step 7, Slice 1 Complete (UI navigation + Upload + Analytics double-write cleanup)

Before starting, audited every `.jsx` file for direct `vanillaState` mutation to scope Step 7 (it touches every domain, so grilled first per CLAUDE.md's >2-file guardrail). Found ~100-116 direct-mutation call sites total, wildly uneven across domains: Sessions/Survey/Report already fully clean (0 sites, routed through dedicated action files from Steps 3-5); Organization (~52-62 sites, no action layer at all, in-place mutation of array elements found via `.find()`, delete-cascade logic duplicated 3x) and Comm (~10-17 sites, no action layer, a 3-levels-deep nested mutation) are the worst offenders; Pulse and Dashboard have moderate debt with no action layer; Upload and the cross-cutting `activeView` page-mount sync had small, well-scoped issues. Confirmed with the user: work this incrementally (matching every other step in this plan), not as one big-bang pass. This slice = the three smallest, most self-contained items; Organization is deliberately deferred to its own future slice given its size and risk.

Completed:

- **`activeView` cross-cutting fix**: all 7 pages (`SessionsPage`, `SurveyPage`, `ReportPage`, `OrgPage`, `DashboardPage`, `PulsePage`, `CommPage`) were writing `vanillaState.activeView = '<page>'; saveState();` directly in their mount effect, even though `useAppStore`'s `setActiveView()` already does exactly that (plus keeps the Zustand mirror in sync, which nothing was calling before). Verified via grep that `vanillaState.activeView`'s only other live reader is Dashboard's own `applyDashboardNavigationState`/`applyDashboardActionState` flow (self-contained, unaffected by this change) — every other reader is inside `app.js`'s `renderView()`/`VanillaCanvas` bridge, both confirmed dead code. Swapped all 7 to `store.setActiveView('<page>')`.
- **Upload action extraction + real bug fix**: created `webapp/src/upload/uploadActions.js` (`setUploadPiiDropped`, `saveUploadedResponses`) — the first dedicated action module for this domain, following the established convention (touches `vanillaState` + `saveState()` only, no Zustand calls inside the action file itself). `UploadPage.jsx`'s `handleSave` was pushing to `state.responses` and firing `saveResponsesToFirestore(...).catch(e => console.error(...))` with no user-facing signal if the cloud write failed — confirmed via CLAUDE.md that localStorage is the app's current source of truth (Firestore is best-effort sync, not yet the backend), so the fix keeps the local-save-succeeds/navigate-away path unchanged but adds a persistent, dismissible warning banner (`vanillaState.uploadSyncWarning`, rendered globally in `AppLayout.jsx` since the triggering page may already be unmounted by the time the Firestore promise settles). Also removed the three now-fully-redundant dynamic `import('../state.js')`/`import('../utils.js')` calls in `handleFileChange`/`handleSave` — `normalizeSessionType` was already statically imported and never needed the dynamic re-import.
- **Found and fixed mid-implementation**: `uploadSyncWarning` initially wasn't in `persistState()`'s explicit save-field whitelist, so it silently reverted to blank on any full state reload (confirmed live via the pre-existing, already-tracked "`?preview=1` drops after idle" bug — reproducing it showed the banner vanishing). Since the whole point of the fix is a warning durable enough to survive navigation, added `uploadSyncWarning` to the whitelist so it now also survives a real page reload, not just SPA navigation.
- **Analytics double-write cleanup**: `AnalyticsPage.jsx`'s `applyFilter()` and `setPhase()` were calling `setSelectedAnalyticsType/Cohort/SessionId/Phase()` (Zustand setters that already write `vanillaState` + call `saveState()` internally) and then redundantly re-writing the same `vanillaState` fields and calling `saveState()` again right after — dead code, zero behavior change to remove. Left `toggleAnalyticsSection`/`setQualAnswersGroupBy` untouched since those have no existing setter to defer to (out of scope for this slice, not a bypassed action).

Verified: `npm run check`, full `vitest run` (64 tests), `npm run build` all pass. Browser-verified live: cycled through all 7 pages and confirmed `vanillaState.activeView` updates correctly via the new path; called `saveUploadedResponses` directly with a Firestore write doomed to fail (unauthenticated preview environment) and confirmed the warning banner renders in `AppLayout`, survives SPA navigation to a different page, survives a full page reload (after the `persistState()` fix — did not survive before it, confirming the fix was necessary), and clears via its dismiss button; applied an Analytics filter post-cleanup with no console errors.
...
### 2026-07-04 - Step 2 + most of Step 8 Complete: app.js retired entirely

Audited `app.js` (1,319 lines) with a dedicated agent before touching anything, given the blast radius (every page still did `import './app.js'` as a pure side-effect import). Finding: exactly one piece of load-bearing logic remained — `window.updateResponsesSubscription` (~95 lines, the chunked Firestore `onSnapshot` listener for `responses`), called from 4 real live modules via `window.updateResponsesSubscription?.()`. Everything else in the file was either unreachable (gated behind `render()`/`renderView()`/`bindCanvasEvents()`, all dead since `window.__reactMode` is permanently `true`) or a duplicate of logic already reimplemented natively in React (org actions, analytics toggles, report qual signals, etc.) with zero remaining `.jsx` call sites.

Completed:

- Extracted the one real symbol as `export function subscribeResponsesFromFirestore()` in `state.js` (alongside the other `subscribe*FromFirestore` functions there), unchanged except one deliberate fix: replaced the `if (shouldRender) render()` gate (which only notified React when `state.activeView` was one of 5 specific views) with an unconditional `saveState()`. That gate was leftover full-DOM-repaint logic from the pre-React app and doesn't apply to React's reactive store sync — keeping it meant Org/Comm/Pulse/Upload pages could silently miss response updates while active on those views. Confirmed via code reading it was never intentional for the React era, not preserved verbatim.
- Rewired all 5 real call sites (`hooks/useInitApp.js` x3, `sessions/sessionActions.js`, `sessions/sessionDraftActions.js`, `survey/surveyResponseActions.js` x3, plus one inside `state.js` itself for `downloadStateFromDb`) from `window.updateResponsesSubscription?.()` to a direct `subscribeResponsesFromFirestore()` import/call.
- Deleted `src/app.js` entirely, plus its two now-fully-vestigial supporting pieces: `src/reactMode.js` (only ever set `window.__reactMode = true`, redundant with `index.html`'s inline script and pointless once nothing checks that flag) and the inline `<script>window.__reactMode = true;</script>` + its comment in `index.html`. Removed `import './app.js'`/`import './reactMode.js'` from `main.jsx`.
- Deleted `components/layout/VanillaCanvas.jsx` (confirmed dead: `VANILLA_VIEWS = []` meant `isVanillaView()` always returned `false`, so it never mounted; its `window.__vanillaRenderView`/`__vanillaBindCanvas` bridge only existed in the now-deleted `app.js`) and its export from `components/layout/index.js`. `AppLayout.jsx`'s canvas div now renders `children` directly, no conditional bridge.
- Rewrote the `app.js`-dependent assertions across 4 test files (`sessionRuntimeWiring`, `surveyRuntimeWiring`, `reportRuntimeWiring`, `uploadRuntimeWiring`) that used `readFileSync` to check `appSource` — all were `.not.toContain(...)` checks that would've been trivially true against a nonexistent file. Kept every assertion that checks other files' current state; replaced the app.js-specific ones with one explicit `expect(existsSync(...app.js)).toBe(false)` test.
...
- Organization is the only real remaining item on the whole plan — Step 7's last domain (~52-62 mutation sites, no action layer, in-place array mutation, delete-cascade logic duplicated 3x). Needs its own dedicated scoping/grilling session given its size and risk, per the original audit's recommendation.

### 2026-07-04 - Step 7 Complete: Organization (last remaining domain) — migration plan finished

Ran a dedicated ground-truth audit before touching anything, given the size and risk (data-model integrity, not just UI toggles). Confirmed: Organization is Firestore-backed via a single shared `appState/main` document (not per-collection like Sessions/Surveys), so every data-model mutation must call `persistOrganization()`, not just `saveState()`, or Firestore sync silently breaks. Confirmed the "3x duplicated delete-cascade" finding from the broader Step 7 audit — and that the 3 copies had quietly diverged: only the team-panel path cleared `selectedDivision/Hq/Team` (and closed the detail panel) when the deleted subtree contained the current selection; the Hq/Division accordion paths did not, a real bug where stale selected-ids could point at now-deleted units.

Completed, as 6 slices (new `webapp/src/org/orgActions.js`, the first action module for this domain):

1. **Nav/UI state**: `setOrgSearchQuery`, `searchOrgAndNavigate`, `clearOrgSearch`, `toggleOrgUnitExpanded`, `selectOrgTeam`, `closeOrgTeamPanel` — moved verbatim from `OrgPage.jsx`.
2. **Reset all org data**: `resetOrganizationData` — moved from `OrgActionMenu.handleReset`.
3. **Save unit / save member**: `saveOrgUnit`, `saveOrgMember` — moved from `UnitEditor`/`MemberEditor`'s `handleSave`. Removed a confirmed dead-code stub in the member-edit path (an empty `forEach` body left over from aborted work — `syncPersonSnapshotsEverywhere`, called right above it, already does the real session-sync job).
4. **Delete member**: `deleteOrgMember` — moved from `OrgTeamPanel.handleDeleteMember`, including the leader-orphan cleanup (clearing `leader`/`leaderTitle`/`leaderMemberId` on any unit that had this member as leader). The confirm-dialog's warning-text computation (which needs the linked-units list *before* the user decides) stays in the component, matching the pattern used for confirm-gated deletes elsewhere in the app.
5. **Delete unit + cascade — the consolidation and bug fix**: `deleteOrgUnitCascade`, replacing all 3 duplicated copies (`OrgTeamPanel`/`AccordionHq`/`AccordionDivision`'s `handleDeleteNode`). Now consistently clears `selectedTeam`/`selectedHq`/`selectedDivision`/`orgSelectedTeamId` for all 3 call sites whenever the deleted subtree contains the current selection — fixing the divergence bug rather than picking one of the three already-inconsistent behaviors to preserve. Also replaced each copy's hand-written recursive `getChildIds` closure with the existing `descendantUnitIds()` helper from `views/org.js`, which did the same thing but was never reused.
6. **Drag-drop reparent**: `reparentOrgUnit`, `reparentOrgMember` — moved from `OrgPage.handleDrop`'s 4-branch dispatch. Discovered mid-extraction that 2 of the 4 branches (team→hq, team→division) were confirmed dead: `AccordionTeam` receives an `onDragStart` prop but never wires it up to a `draggable`/`onDragStart` attribute, so nothing in the UI ever sets the drag type to `'team'` — only Hq units, Division units, and Members are actually draggable. Dropped the 2 dead branches rather than preserving unreachable code, matching this migration's established pattern (e.g. the earlier Sessions calendar bug, the Pulse `QUESTIONS`/`companyFav` imports).

`OrgPage.jsx` and `OrgComponents.jsx` no longer import `saveState`/`persistOrganization`/`syncDraftOrgFromTeam`/`syncPersonSnapshotsEverywhere`/`descendantUnitIds` directly (all moved into the action module); cleaned up several other imports (`hqUnitsForDivision`, `teamUnitsForSelection`, `escapeHtml`) that turned out to already be unused before this refactor even started.
